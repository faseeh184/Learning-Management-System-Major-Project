...
const fetchHadiyaReportAPI = async (params) => {
  ...
  try {
    return await authFetch(`VITE_API_URL/hadiya/report?${queryString}`);
  } catch (error) {
...
const recordHadiyaPaymentAPI = async (paymentData) => {
  try {
    return await authFetch('VITE_API_URL/hadiya/record', {
      method: 'POST',
...
const HadiyaManagement = () => {
  ...
  useEffect(() => {
    const fetchCenters = async () => {
      try {
        const data = await authFetch('VITE_API_URL/centers');
        setCenters(data);
      } catch (error) {
