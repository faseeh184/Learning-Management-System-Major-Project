...
const Reports = () => {
  ...
  useEffect(() => {
    const fetchCenters = async () => {
      try {
        const response = await fetch('VITE_API_URL/centers', {
          headers: {
    ...
    const fetchAttendanceData = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `VITE_API_URL/attendance/report?month=${month}&year=${year}&centerId=${centerId}`,
          {
            headers: {
              'Authorization': `Bearer ${JSON.parse(localStorage.getItem('userData')).token}`
            }
  ...
  const handleMarkAttendance = async (tutorId, date, status) => {
    try {
      const response = await fetch('VITE_API_URL/attendance/mark', {
        method: 'POST',
        headers: {
