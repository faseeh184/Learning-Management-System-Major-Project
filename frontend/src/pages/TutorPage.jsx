...
const TutorPage = () => {
  ...
  const refreshLocation = async () => {
    ...
    try {
      ...
      const result = await post("VITE_API_URL/auth/tutor/login", payload);
  ...
  const sendLocationToBackend = async (token) => {
    ...
    try {
      const currentLocation = await getCurrentLocation();
      await post("VITE_API_URL/tutors/update-location", { currentLocation, token });
  ...
  const submitAttendance = async () => {
    ...
    try {
      ...
      const result = await post("VITE_API_URL/tutors/attendance", {
        currentLocation,
        token
      });
